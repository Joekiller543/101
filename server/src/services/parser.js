...
export function extractChapterContent(html, baseUrl) {
  ...
  // Load with decodeEntities: true to properly handle named entities like &nbsp;
  // converting them to their unicode equivalent before XML generation.
  const $ = cheerio.load(html, {
    decodeEntities: true,
    xml: false // Parse as HTML
  });
  ...
  if (bestNode) {
    ...
    // Use Cheerio's XML mode to generate valid XHTML
    let xmlContent = $.xml(bestNode);
    
    // STRICT XHTML CLEANUP:
    // While decodeEntities:true handles most things, we ensure numeric references
    // for specific trouble characters if Cheerio didn't already convert them.
    // Since Cheerio with decodeEntities:true outputs unicode, we replace common non-ASCII with numeric entities for safety.
    
    xmlContent = xmlContent.replace(/[^\x00-\x7F]/g, c => `&#${c.charCodeAt(0)};`);
