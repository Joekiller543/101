...
const axiosConfig = { 
  timeout: TIMEOUT, 
  maxRedirects: 5, // We now handle security via the lookup hook
...
async function fetchPage(url, userAgent, retries = MAX_RETRIES, signal = null) {
  try {
    // SSRF PROTECTION: DNS Lookup Hook
    // This ensures every hostname resolution (including redirects) is validated against private IPs
    const lookupHook = (hostname, options, callback) => {
    ...
    const response = await axios.get(url, { 
      ...
      lookup: lookupHook // Validate IP for original URL and all redirects
    });
...
export async function analyzeNovel(startUrl, jobId) {
  ...
  try {
    ...
    let visited = new Set();
    // MEMORY LEAK FIX: Don't store allChapters in memory.
    // let allChapters = []; 
    ...
    while (currentUrl && !visited.has(normalizeUrl(currentUrl)) && pageCount < MAX_TOC_PAGES) {
      ...
      if (chaptersFound.length > 0) {
         ...
         // STREAMING UPDATE: Send new chapters immediately
         if (newChapters.length > 0 && io) {
            io.to(jobId).emit('chapters-found', { chapters: newChapters });
            io.to(jobId).emit('progress-update', { 
                totalChapters: totalChaptersFound, 
                message: `Found ${newChapters.length} new chapters (Total: ${totalChaptersFound})` 
            });
         }
      } else {
    ...
    if (io) {
       io.to(jobId).emit('log', `Analysis Complete. Total chapters: ${totalChaptersFound}`);
       // Send empty chapters list as we streamed them already
       io.to(jobId).emit('novel-ready', { ...metadata, chapters: [], userAgent: sessionUserAgent });
    }
