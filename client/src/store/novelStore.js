...
import { saveChapter, deleteJobChapters } from '../lib/db';
...
export const useNovelStore = create((set, get) => ({
  ...
  initSocket: () => {
    ...
    // Handle streamed chapters
    newSocket.on('chapters-found', (data) => {
      if (data.chapters && data.chapters.length > 0) {
        set((state) => ({
           chapters: [
             ...state.chapters, 
             ...data.chapters.map(c => ({ ...c, status: 'pending', selected: true }))
           ]
        }));
      }
    });
    ...
    newSocket.on('novel-ready', async (data) => {
      ...
      // Note: If chapters were streamed, 'chapters' array here might be empty to save bandwidth
      // So we only replace current chapters if the incoming list is not empty
      
      if (chapters && chapters.length > 0) {
        // Clear previous chapters for THIS job only to prevent collisions/bloat
        await deleteJobChapters(jobId);
      ...
      const totalCount = get().chapters.length;
      get().addLog(`Analysis complete. Found "${metadata.title}" with ${totalCount} chapters.`);
    });
  ...
  analyzeNovel: async () => {
    ...
    // Clear existing chapters before new analysis
    await deleteJobChapters(jobId);
    
    set({ status: 'ANALYZING', error: null, logs: [], chapters: [] });
  ...
  fetchChapters: async () => {
    ...
    // pendingItems is an array of objects: { index, title, url }
    ...
    const fetchBatch = async () => {
      ...
      try {
        ...
        set((state) => {
          const freshChapters = [...state.chapters];
          
          data.results.forEach((result, idx) => {
             const originalIdx = batchItems[idx].index;
             // Even if state.chapters grew (due to streaming), originalIdx points to the correct slot 
             // (assuming appending only, which streaming does)
             // If array shifted, we might have issues, but appending is safe.
             
             if (freshChapters[originalIdx]) {
                 if (result.success) {
                     // Side Effect: Save to DB
                     saveChapter({ url: result.url, content: result.content, title: result.title }, jobId);
  ...
  startGeneration: async () => {
    const { novelMetadata, chapters, addLog, jobId } = get();
    ...
    try {
      ...
      }, jobId); // Fix: Pass jobId so worker can find content
