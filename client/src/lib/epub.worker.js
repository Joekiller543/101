...
self.onmessage = async (e) => {
  const { novel, jobId } = e.data;
  try {
    const blob = await generateEpub(novel, jobId);
...
async function generateEpub(novel, jobId) {
  const zip = new JSZip();
  const uuid = 'urn:uuid:' + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString());
  const date = new Date().toISOString().split('T')[0];

  // Open IDB
  const db = await openDB(DB_NAME, 2); // Version 2 matched with db.js
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  ...
  for (let i = 0; i < novel.chapters.length; i++) {
    ...
    // Fetch content from IDB using Compound Key
    let contentHtml = '<p>Content missing.</p>';
    try {
       // Fix: Retrieve using [jobId, url]
       const record = await db.get(STORE_NAME, [jobId, chapterMeta.url]);
